...
import requests
import datetime
from telebot.aync_telebot import AsyncTeleBot
...
firebase_admin.initialize_app(cred, { 'storage_bucket': os.getenv('tgapp-8404d.appspot.com') })
db = firestore.client()
...
async def start(message):
  ...
  is_premuim = message.from_user.is_premium
  ...
  try:
      ...
      if not user_doc.exists:
          ...
          if response.status_code == 200:
              ...
              user_image = blob.generate_signed_url(datetime.timedelta(days=365),
                  method="GET"
              )
          else:
              user_image = None
          ...
          if len(text) > 1 and text[1].startswith('ref='):
              ...
              if referrer_doc.exists:
                  ...
                  bonus_amount = 500 if is_premuim else 100
...
async def process_update(update_dict):
    ...
    def do_GET(self):
        self.send_response(200)
        self.end_headers()
        self.wfile.write(b'Webhook is working!'.encode)
